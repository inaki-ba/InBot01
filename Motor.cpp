...
MyMotor::MyMotor( uint8_t ENA, uint8_t ENB, uint8_t INA1, 
	uint8_t INA2, uint8_t INB1, uint8_t INB2)
{
     ...
     if (motor_->init(&init) != COMPONENT_OK) {
      ...
      motor_->set_dual_full_bridge_config(PARALLELING_NONE___2_UNDIR_MOTOR_BRIDGE_A__2_UNDIR_MOTOR_BRIDGE_B);
 
      /* Set PWM Frequency of bridge A inputs to 1000 Hz */ 
      motor_->set_bridge_input_pwm_freq(0,1000);
  
      /* Set PWM Frequency of bridge B inputs to 2000 Hz */ 
      motor_->set_bridge_input_pwm_freq(1,2000);
